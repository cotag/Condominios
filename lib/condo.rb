			...
			def create
				...
				if upload.present?
					...
					if upload.resumable_id.present? && upload.resumable
						upload.object_options[:parameters] =  ({} || upload_params[:object_options][:parameters]).merge(upload.object_options[:paramters]) if upload_params[:object_options].present? && upload_params[:object_options][:parameters].present?	# This seems more secure (May need to request the next set of parts)
					...
					if !!valid
						...
						# Build the request
						#
						request = residence.new_upload(upload_params.merge!({
